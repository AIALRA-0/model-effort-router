...
审计结论：`reframe`。保留模型路由器，但把当前项目从“先写研究结论”改成“先在真实项目中收集经同意、可验证、隐私最小化的执行统计”。以 `0.2.0` 作为独立公开试点版本；在达到复核门槛前，不发布模型胜率、固定消耗倍率或普适最佳档位。
...
- 当前宿主能够稳定提供哪些 token、缓存 token 和推理 token 字段仍然未知。
- 50 次运行是否足以支持任何具体比较仍然未知；它只代表第一次人工复核时间点。
- 当前审计已经取得公开先例和用户决策所有者指令，但尚未取得独立人员对本仓库隐私实现的代码审查。

## 9 推荐

推荐发布 `0.2.0` 公开试点并安装到本机，启用仅限本机的结构化采集，从下一次完整项目迭代开始记录。当前任务已经过半后才完成采集器，因此不补写本轮记录，避免把不完整观测混入真实样本。
